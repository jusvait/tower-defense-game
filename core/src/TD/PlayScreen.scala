...
/** New game / Load game screen */
class PlayScreen(var game: TowerDefense) extends Screen {
  ...
  var newGameButton = new TextButton("New Game", playButtonStyle)
  ...
  var loadGameButton = new TextButton("Load Game", playButtonStyle)

  loadGameButton.addListener(new InputListener(){
    override def touchDown(event: InputEvent, x: Float, y: Float, pointer: Int, button: Int) = {
      if (Gdx.files.local("Save").exists()) {
        Serializer.loadSave(game,"Save")
      } else Serializer.loadLevel(game, "Level_01") // If there is no save file, just load first level
  ...
  table.add(loadGameButton).padBottom(30)
